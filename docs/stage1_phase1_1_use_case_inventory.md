...
## 1.1.2.1 Application Wiring Responsibility Map

This section maps the real wiring and composition responsibilities inside the current application layer. It is documentary only and does not authorize Go source changes, tests changes, configuration changes, refactors, file moves, function renames, route changes, provider changes or behavior corrections.

### Current Wiring Map

| Finding | Classification | File | Function or method | Wiring responsibility | Incoming dependencies | Outgoing dependencies | Affected module or layer | Correctly located | Should remain in application layer | Deferred phase |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| WIR-001 | A) wiring correct | `cmd/scavo-server/main.go` | `main` | Load environment config, create `internal/app.App`, start it, wait for shutdown signal and call `Stop`. | OS process, environment variables, signal context. | `config.LoadFromEnv`, `app.New`, `App.Start`, `App.Stop`. | Process entrypoint and runtime app. | Yes. It is process bootstrap, not product behavior. | Yes, as entrypoint-to-runtime connection only. | No Phase 1.1 implementation target. |
| WIR-002 | B) wiring coupled but acceptable | `internal/app/app.go` | `New` | Build logger, WebSocket hub/dispatcher, token service, database/cache clients, repositories, module services, auth provider, status service, router and HTTP server. | `config.Config`. | `core/logger`, `core/ws`, `core/auth`, `core/db`, `core/cache`, `core/status`, `core/httpx`, `auth`, `user`, `usersettings`, `system`. | Runtime composition root across core and modules. | Yes. Coupling is expected for a composition root. | Yes, but only for construction and lifecycle wiring. | Phase 1.1 documentation guardrail; no code correction now. |
| WIR-003 | A) wiring correct | `internal/app/app.go` | `New` | Register system WebSocket actions before router/server construction. | `ws.Dispatcher`. | `system.Register`. | `system` transport over `core/ws`. | Yes. Registration is runtime wiring. | Yes, as module registration only. | Phase 1.6 only if behavior consistency is later audited. |
| WIR-004 | E) requires later review | `internal/app/app.go`; `internal/modules/auth/ws_handlers.go` | `New`; `auth.RegisterWS` | Register auth WebSocket actions with a root `auth.Service` support service. | `ws.Dispatcher`, `auth.Service`, token service, user service. | `auth.whoami`, `auth.session`, `core/ws.RequireAuth`, `Service.ResolveSessionClaims`. | Auth realtime transport and session support. | Partially. Runtime registration is correct, but WS session support still uses root compatibility service. | Registration should remain in runtime app; final WS intent mapping is deferred. | Phase 1.1.2 flow mapping documentation; Phase 1.6 behavior audit if payload consistency changes are considered. |
| WIR-005 | B) wiring coupled but acceptable | `internal/app/app.go` | `New` | Initialize JWT token service from config and fallback to development token settings on invalid config. | `cfg.JWTSecret`, `cfg.JWTIssuer`, `cfg.JWTTTLHrs`, logger. | `coreauth.NewTokenService`. | Core auth runtime dependency. | Yes as operational bootstrapping. | Yes, but fallback semantics must not be treated as product auth policy. | Phase 1.3 only if auth/permission policy work needs it. |
| WIR-006 | B) wiring coupled but acceptable | `internal/app/app.go` | `New` | Initialize Postgres and Redis clients and retain startup errors for readiness checks. | `config.Config`, logger, init context. | `db.New`, `cache.New`, `status.FuncChecker`. | Core infrastructure and diagnostics. | Yes. Infrastructure clients are created at composition root. | Yes, as infrastructure wiring only. | No Phase 1.1 implementation target. |
| WIR-007 | B) wiring coupled but acceptable | `internal/app/app.go` | `New` | Select Postgres-backed user/usersettings repositories when DB is available, otherwise construct nil-backed services. | DB client/pool, logger. | `user.NewPostgresRepository`, `user.NewService`, `usersettings.NewPostgresRepository`, `usersettings.NewService`. | `user` and `usersettings` module application services. | Yes for construction; behavior remains inside modules. | Yes, while repository behavior and defaults stay outside `internal/app`. | Phase 1.2/1.5 only if account/settings behavior changes are scoped. |
| WIR-008 | B) wiring coupled but acceptable | `internal/app/app.go` | `New` | Select Postgres-backed or in-memory wallet challenge and identity stores. | DB client/pool. | `auth.NewWalletChallengeStorePG`, `auth.NewWalletIdentityStorePG`, `auth.NewInMemoryWalletChallengeStore`, `auth.NewInMemoryWalletIdentityStore`. | Auth wallet persistence/support stores. | Yes for runtime store selection. | Yes, as store construction only. | Phase 1.2.3 or Phase 1.6 if wallet behavior is later changed. |
| WIR-009 | E) requires later review | `internal/app/app.go`; `internal/modules/auth/application.go`; `internal/modules/auth/app/application.go`; `internal/modules/auth/provider.go` | `New`; `auth.NewApplication`; `authapp.NewApplication` | Build the handler-facing `AuthProvider` from tokens, user service, settings service, public base URL, challenge TTL and wallet stores. | Token service, TTL, module services, config, stores. | Root `auth.Application`, inner `auth/app.Application`, auth domain provider contracts. | Auth provider/application boundary. | Yes. This is the current provider-first Stage 0 wiring. | Yes, but the root wrapper must stay documented as compatibility, not primary behavior owner. | Phase 1.1.5 contract documentation; implementation-sensitive cleanup deferred. |
| WIR-010 | A) wiring correct | `internal/app/app.go`; `internal/core/httpx/router.go` | `New`; `httpx.NewRouter` | Pass consolidated router dependencies and create the HTTP server handler. | Logger, hub, dispatcher, config, token service, status service, auth provider. | `RouterParams`, `chi.Router`, `ws.NewHandler`, auth HTTP handlers. | Core HTTP/runtime routing. | Yes. Router receives provider-level auth dependency instead of lower-level module internals. | Yes for transport/runtime wiring; product orchestration must remain behind provider/application methods. | No Phase 1.1 code target; behavior audit deferred to Phase 1.6 if needed. |
| WIR-011 | A) wiring correct | `internal/core/httpx/router.go` | `registerAuthRoutes` | Register legacy and `/api/v1` auth routes with auth and authorization middleware. | Router prefix, token service, auth HTTP handlers. | `RequireAuth`, `HydrateAuthorization`, `RequirePermission`, handler methods. | HTTP transport and Stage 0 frozen route surface. | Yes as transport route wiring. | No product logic should be added here; route wiring remains core/http responsibility. | Phase 1.3 for deeper authorization refinement; Phase 1.6 for cross-endpoint behavior consistency. |
| WIR-012 | A) wiring correct | `internal/app/app.go` | `Start`; `Stop` | Start WebSocket hub and HTTP server; stop database/cache/server resources. | `App` fields, runtime context. | `hub.Run`, `server.ListenAndServe`, `dbClient.Close`, `cacheClient.Close`, `server.Shutdown`, lifecycle flow logs. | Runtime lifecycle. | Yes. Lifecycle is not module behavior. | Yes, as application runtime lifecycle only. | No Phase 1.1 implementation target. |
| WIR-013 | C) wiring dispersed | `internal/modules/auth/app/application.go`; `internal/modules/auth/app/wallet_services.go` | `Application` methods; wallet service constructors | Build short-lived wallet challenge/link/merge/primary/detach support services inside auth application methods. | Auth application fields, challenge store, identity store, public base URL, challenge TTL. | `NewWalletChallengeService`, `NewWalletLinkingService`, `NewWalletAccountMergeService`, `NewWalletPrimaryService`, `NewWalletDetachService`. | Auth module application/support services. | Acceptable inside auth module, but dispersed across application methods and wallet support services. | It should remain in auth module application/support layer, not `internal/app`. | Phase 1.1 flow mapping/contract docs; Phase 1.2.3 only if wallet usability behavior changes. |
| WIR-014 | D) responsibility outside of place risk | `internal/modules/auth/app/application.go`; `internal/modules/user/app/service.go`; `internal/modules/usersettings/app/service.go` | `UpdateProfile`; `GetSettings`; `UpdateSettings`; `GetBootstrap` | Auth application wires calls into user and usersettings providers for authenticated account/settings exposure. | Claims, user provider, settings provider, wallet identity store. | `UserProvider.UpdateDisplayName/GetByID`, `UserSettingsProvider.GetOrDefault/UpdatePreferences`, mappers. | Cross-module auth/account/settings composition. | Partially. Auth owns authenticated exposure, while behavior owners remain `user` and `usersettings`. | The cross-module composition may remain at auth application boundary; validation/default/merge behavior must remain in owning modules. | Phase 1.1.5 for contract docs; Phase 1.2/1.5 for product behavior changes. |

### Wiring Classification Summary

- A) Wiring correct: WIR-001, WIR-003, WIR-010, WIR-011 and WIR-012.
- B) Wiring coupled but acceptable: WIR-002, WIR-005, WIR-006, WIR-007 and WIR-008.
- C) Wiring dispersed: WIR-013.
- D) Responsibility outside of place risk: WIR-014.
- E) Requires later review: WIR-004 and WIR-009.

### Wiring Risks Detected

- `internal/app.New` has expected composition-root coupling to many core and module packages; future product logic added there would create excessive application-layer ownership.
- Runtime fallback wiring for token service, repositories and stores can be mistaken for domain defaults if not kept documented as operational compatibility.
- WebSocket auth wiring still uses the root compatibility auth service while HTTP uses the provider boundary, so HTTP/WS identity and session intent must remain explicitly mapped before behavior changes.
- Auth provider construction is clear but still crosses a root compatibility wrapper and an inner `auth/app.Application`; future edits can target the wrong owner.
- Router-level auth and authorization middleware is correctly placed, but deep permission semantics belong to later authorization phases, not application wiring consolidation.
- Cross-module calls from auth into user and usersettings are legitimate for authenticated exposure, but ownership of profile validation and settings defaults/merge must remain outside auth.
- Wallet support-service construction is dispersed inside auth application methods; it is not a runtime composition-root problem, but it should be mapped before any wallet usability consolidation.

No wiring is corrected in 1.1.2.1. No Go source, tests or configuration changes are authorized by this responsibility map.

## 1.1.2.2 Application Boundary Risk Review

This section reviews boundary risks in and around the current application layer before any future consolidation work. It is documentary only and does not authorize Go source changes, tests changes, configuration changes, refactors, file moves, function renames, provider changes, authorization changes, route changes or risk corrections.

### Boundary Risk Register

| Risk | Classification | Area | File(s) | Description | Potential impact | Phase 1.1 impact | Future recommendation | Resolve in Phase 1.1 or defer |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ABR-001 | A) Low | Process entrypoint boundary | `cmd/scavo-server/main.go` | Entrypoint loads config, creates `internal/app.App`, starts/stops runtime and does not host product or module behavior. | Low risk of boundary drift unless future process code starts orchestrating use cases directly. | Does not block Phase 1.1; keep as documented runtime entrypoint. | Preserve `cmd` as process bootstrap only. | No Phase 1.1 resolution required. |
| ABR-002 | B) Medium | Runtime composition root | `internal/app/app.go` | `App.New` necessarily imports and constructs many core and module dependencies. This is valid for a composition root but highly coupled by design. | Future additions could turn `internal/app` into an accidental product owner or domain orchestration location. | Affects Phase 1.1 as a guardrail: flow owners should point to module app/provider boundaries, not `internal/app`. | Keep `internal/app` limited to construction, registration and lifecycle; document any future dependency additions explicitly. | Phase 1.1 documentation only; implementation deferred unless a later subphase scopes composition cleanup. |
| ABR-003 | B) Medium | Runtime fallback decisions | `internal/app/app.go`; `internal/modules/user/app/service.go`; `internal/modules/usersettings/app/service.go`; `internal/modules/auth/*store*` | DB-disabled startup creates nil-backed user/settings services and in-memory wallet stores; token setup has a development fallback on invalid JWT config. | Operational fallback behavior may be mistaken for product defaults or domain policy. | Affects Phase 1.1 only as boundary documentation; changing fallback behavior would reopen runtime/contract expectations. | Keep fallback decisions documented as environment/runtime compatibility and keep domain defaults in owning modules. | Defer behavior changes to later roadmap-approved runtime or product phases. |
| ABR-004 | B) Medium | Repository boundary | `internal/app/app.go`; repository packages under `internal/modules/*/repository*` | `internal/app` selects repository/store implementations but does not execute repository queries. Repository behavior remains in module repository/store packages. | Risk is low-to-medium today; future inline persistence logic in `internal/app` would violate module boundaries. | Does not require Phase 1.1 correction; confirms the current boundary is acceptable. | Continue constructing repositories in composition root while keeping persistence logic in repositories/stores. | No Phase 1.1 resolution; watch in later implementation phases. |
| ABR-005 | B) Medium | Provider boundary | `internal/app/app.go`; `internal/modules/auth/application.go`; `internal/modules/auth/app/application.go`; `internal/modules/auth/provider.go` | Production HTTP wiring uses `AuthProvider`, but concrete construction crosses the root compatibility `auth.Application` wrapper and inner `auth/app.Application`. | Future edits may target the wrapper as primary owner or bypass the provider boundary, creating drift between compatibility and active use-case owner. | Relevant to Phase 1.1 naming/contract documentation. No code change is allowed here. | Document `auth/app.Application` as active use-case owner and root `auth.Application` as compatibility/provider adapter. | Phase 1.1.5 documentation; implementation-sensitive cleanup deferred. |
| ABR-006 | B) Medium | HTTP/router and authorization boundary | `internal/core/httpx/router.go`; `internal/core/authorization/*` | Router attaches `RequireAuth`, `HydrateAuthorization` and selected `RequirePermission` middleware. Deep policy semantics are not in `internal/app`, but route-level choices are close to product surfaces. | Authorization semantics could be changed accidentally from router wiring during application consolidation. | Phase 1.1 may document route-to-flow intent only; deep authorization decisions are out of scope. | Keep router as transport/middleware boundary and move authorization product refinement to Phase 1.3. | Defer to Phase 1.3 unless only documentation is needed in Phase 1.1. |
| ABR-007 | C) High | Auth application and wallet behavior boundary | `internal/modules/auth/app/application.go`; `internal/modules/auth/app/wallet_services.go` | Wallet challenge, signature, link, merge, primary and detach rules live in auth module application/support services rather than in pure domain services. | High risk during future consolidation: moving or renaming these flows without a map could change wallet behavior or blur domain/application responsibility. | Phase 1.1 must only map and document these flows; it must not split or relocate behavior. | Document wallet flow ownership and challenge-purpose taxonomy before Phase 1.2 wallet usability changes. | Phase 1.1 documentation; behavior/refactor deferred to Phase 1.2.3 or Phase 1.6. |
| ABR-008 | B) Medium | Cross-module account/settings boundary | `internal/modules/auth/app/application.go`; `internal/modules/auth/domain/user_contract.go`; `internal/modules/auth/domain/usersettings_contract.go`; `internal/modules/user/app/service.go`; `internal/modules/usersettings/app/service.go` | Auth application exposes authenticated profile/settings flows while user and usersettings services own validation, defaults, merge and persistence-facing behavior. | Auth could absorb account/settings behavior during consolidation, weakening module ownership. | Phase 1.1 should document authenticated exposure versus behavior ownership only. | Keep profile mutation semantics in `user` and settings semantics in `usersettings`; document auth as orchestrator/exposure. | Phase 1.1.5 documentation; product behavior deferred to Phase 1.2/1.5. |
| ABR-009 | B) Medium | WebSocket versus HTTP boundary | `internal/app/app.go`; `internal/modules/auth/ws_handlers.go`; `internal/modules/auth/service.go`; `internal/core/ws/*`; `internal/core/httpx/router.go` | HTTP auth routes use provider/application responses, while WS `auth.whoami` builds a session map directly and WS `auth.session` uses root auth service. | Client-visible session/current-user state can drift across transports if not mapped before changes. | Phase 1.1 should classify intent and differences, but not alter WS payloads or wiring. | Treat WS auth actions as separate transport surfaces until a later behavior audit confirms alignment expectations. | Phase 1.1 mapping/documentation; behavior deferred to Phase 1.6. |
| ABR-010 | B) Medium | Bootstrap aggregation boundary | `internal/modules/auth/app/application.go`; `internal/modules/user/app/service.go`; `internal/modules/usersettings/app/service.go`; `internal/modules/auth/app/support_helpers.go` | Bootstrap aggregates session, user/profile, settings and wallets through auth application. It is a read aggregation, not downstream domain ownership. | Aggregator growth could hide ownership transfer and make later account/settings/wallet changes harder to reason about. | Directly relevant to Phase 1.1 flow mapping; no implementation correction belongs here. | Document field-level owners for bootstrap before adding account or settings product capabilities. | Phase 1.1 documentation; behavior changes deferred to Phase 1.2+. |
| ABR-011 | A) Low | System module boundary | `internal/app/app.go`; `internal/modules/system/ws_handlers.go`; `internal/core/ws/*` | `system.Register` wires a minimal `system.ping` action into the dispatcher. No product or domain behavior is mixed into `internal/app`. | Low risk unless system actions grow into product diagnostics or business behavior without a boundary. | Does not block Phase 1.1; keep as runtime/system transport note. | Keep system WS behavior minimal and route any future diagnostics/system changes through their own roadmap scope. | No Phase 1.1 resolution required; later Phase 1.6 only if behavior consistency changes. |
| ABR-012 | D) Deferred / not applicable for Phase 1.1 | Deep product authorization and ownership semantics | `internal/core/authorization/*`; `internal/core/httpx/router.go`; wallet/user/settings modules | Account-level ownership checks, refined permissions and product authorization semantics are Stage 1 concerns but not Phase 1.1 implementation work. | Changing them during application boundary review would invade Phase 1.3 or Phase 1.2.5 and could break Stage 0 contracts. | Phase 1.1 should only record the boundary and defer. | Resolve in dedicated authorization/product phases with explicit validation. | Defer to Phase 1.3 and Phase 1.2.5. |

### Boundary Risk Summary

- Low risks: ABR-001 and ABR-011.
- Medium risks: ABR-002, ABR-003, ABR-004, ABR-005, ABR-006, ABR-008, ABR-009 and ABR-010.
- High risks: ABR-007.
- Deferred or not applicable for Phase 1.1: ABR-012.

### Phase 1.1 Boundary Decision

- No verified repository logic is currently implemented inside `internal/app`; it constructs repositories and stores only.
- No direct product domain behavior is currently implemented in `cmd/scavo-server` or `internal/app`; the main risk is future drift because `internal/app.New` is broad by necessity.
- Domain-adjacent wallet rules exist inside the auth module application/support layer, not in `internal/app`; this is a high-risk consolidation area and must remain mapping/documentation-only in Phase 1.1.
- Router-level authorization wiring is present, but deep policy and ownership semantics must be deferred to authorization/product phases.
